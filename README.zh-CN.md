...
> **当前状态：** Proto UI 处于 v0 预发布阶段。精确版本 `0.2.0-rc.1` 已通过 npm `next` channel 发布，可用于可复现试用；它不是稳定的 `latest` 上手路径，也不推荐用于关键生产业务。
...
完整的生成路径、样式引入、带类型组件使用、多宿主方式和当前限制见 [0.2 RC 试用](https://proto-ui.com/zh-cn/start-here/rc-trial/)。不可变发行证据见 [v0.2.0-rc.1 Release](https://github.com/Proto-UI/Proto-UI/releases/tag/v0.2.0-rc.1)。
...
当前工作的重点是：在真实项目中试用 `0.2.0-rc.1`，修复上手或语义 blocker，按照试用暴露的真实缺口补充文档，并在不无序扩张 Prototype 表面的前提下完成 `0.2` release line 收口。
