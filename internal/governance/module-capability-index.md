...
| `@proto.ui/module-expose-state` | Required | Finalizes the complete expose registry for Adapter publication and attenuates internal State into external handles | `EXPOSES_RECORD_SINK_CAP` (legacy `EXPOSE_STATE_SET_EXPOSES_CAP` is a deprecated alias) | `@proto.ui/module-expose`, `@proto.ui/module-state` | Exposed State loses the unified subscription model and the complete record loses its standard publication path | This module owns the complete finalized expose sink and should consume expose/state outputs rather than re-requesting the same upstream host meaning |
