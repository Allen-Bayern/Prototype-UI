...
| `@proto.ui/module-text-control` | Optional | Supports declared native multiline text controls with stable controlled/uncontrolled ownership, normalized input/change/IME events, live property projection, and physical focus access | `TEXT_CONTROL_HOST_CAP`, `TEXT_CONTROL_RUN_IN_CALLBACK_CAP` | `@proto.ui/module-base` | Prototypes using the Text Control domain cannot materialize or synchronize a host-native control | Wire only for declared text-control targets; web adapters may use `createWebTextControlHost`, while wrapper hosts must contain physical native events before publishing normalized prototype events |
