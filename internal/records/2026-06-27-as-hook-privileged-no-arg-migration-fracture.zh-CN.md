...
- `asFocusScope()` 与 `asFocusRoving()` 已迁移为无参数 caller；setup-time 配置分别通过返回的 `FocusScopeHandle.configure(...)` 与 `FocusRovingHandle.configure(...)` 完成。
- `asOverlay(patch)`、`asBoundary(patch)`、`asHitParticipation(...)`、`asTransition(options)` 仍带参数化或 configurable 行为。
...
- Done: `asFocusScope` and `asFocusRoving` caller shapes migrated to no-arg; scope/roving configuration now goes through returned handles.
