---
title: '0.2 RC 试用'
description: '使用精确的 0.2.0-rc.1 版本验证 Proto UI 上手流程'
---
...
<DocStageNotice
  tone="amber"
  title="这是独立的 0.2.0-rc.1 预发布试用流程。"
  body="它不会改变官网主 Quick Start 对 npm latest 稳定版本的承诺。"
/>

:::caution[预发布试用] `0.2.0-rc.1` 现已发布到 npm。试用发现不代表稳定版承诺，命令和生成结构仍可能在 `0.2.0` 前调整。:::

## 安装 Proto UI CLI

你可以直接通过 `npx` 使用 Proto UI CLI，不需要提前全局安装。本文固定使用 `0.2.0-rc.1`，以便试用结果可以复现；CLI 会将 Adapter 与 Prototype package 固定到和自身完全一致的版本，避免混用历史 release。
...
生成的 Shadcn theme 在应用没有声明主题时会跟随系统的 `prefers-color-scheme`。如果应用自行管理主题，请在根元素上设置 `data-theme="light"` / `data-theme="dark"`，或使用兼容的 `.light` / `.dark` class；显式主题会覆盖系统偏好。
...
```bash
npx @proto.ui/cli@0.2.0-rc.1 add react shadcn-button --no-install
```
...
生成的组件会保留 Prototype 声明的 props、事件 listener 与 ref/expose surface。在 TypeScript 项目中，`variant="outline"` 等支持值会获得补全，非法 variant 与未知 prop 会被类型检查拒绝。
...
```bash
npx @proto.ui/cli@0.2.0-rc.1 add react shadcn-toggle
npx @proto.ui/cli@0.2.0-rc.1 add react shadcn-dialog
```
...
```bash
npx @proto.ui/cli@0.2.0-rc.1 add react shadcn-button
npx @proto.ui/cli@0.2.0-rc.1 add wc shadcn-button
```
