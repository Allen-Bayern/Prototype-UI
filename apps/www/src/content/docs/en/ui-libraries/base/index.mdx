...
`textarea` exercises a native-control boundary: Root owns exactly one physical native textarea while its renderer stays contentless. It keeps controlled/uncontrolled ownership stable, normalizes input/change/IME signals, restores unaccepted controlled proposals after composition, and projects native props, focus, and accessibility to the same target without claiming form submission, rich text, auto-resize, or live-region semantics.
