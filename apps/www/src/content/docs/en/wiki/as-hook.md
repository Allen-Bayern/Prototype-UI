...
- `useCollection()` and `useCollectionItem()` build ordered collection structure semantics on top of that view

This is important for Proto UI's layering:

- `anatomy.order` remains an enhancement of structural capability
- `useCollection` remains a reusable structure helper built on top of that capability

That boundary matters: `useCollection` should answer who belongs to the ordered set and what each item's structural position is. It should not own roving focus, active item policy, or selection.
