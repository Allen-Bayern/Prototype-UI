...
To verify installation, generation, and component usage with the published `0.2.0-rc.7`, use the separate [0.2 RC Trial](/en/start-here/rc-trial/). That page pins the exact RC version, keeping trial results reproducible when the `next` dist-tag moves.
