---
title: '0.2 RC Trial'
description: 'Verify the Proto UI onboarding flow with the exact 0.2.0-rc.6 release'
---
...
<DocStageNotice
  tone="amber"
  title="This is the isolated 0.2.0-rc.6 prerelease trial flow."
  body="It does not change the main Quick Start contract, which follows the stable npm latest release."
/>

:::caution[Prerelease trial] `0.2.0-rc.6` is now available from npm. Trial findings are not stable-version promises, and commands or generated structure may still change before `0.2.0`. :::

## Install the Proto UI CLI

You can use the Proto UI CLI directly with `npx`. There is no need to install it globally in advance. This page pins `0.2.0-rc.6` so trial results remain reproducible; the CLI pins Adapter and Prototype packages to its own exact version so historical release trains cannot be mixed in.
...
```bash
npx @proto.ui/cli@0.2.0-rc.6 add react shadcn-button --no-install
```
...
```bash
npx @proto.ui/cli@0.2.0-rc.6 add react shadcn-toggle
npx @proto.ui/cli@0.2.0-rc.6 add react shadcn-dialog
```
...
```bash
npx @proto.ui/cli@0.2.0-rc.6 add react shadcn-button
npx @proto.ui/cli@0.2.0-rc.6 add wc shadcn-button
```
