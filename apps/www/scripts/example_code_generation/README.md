...
1. **生成 prototype-config.ts**：扫描所有 `.demo.ts` 中的 `prototypeId`，按前缀映射到对应组件库（如 `shadcn` → `@prototype-libs/shadcn`），并将 `prototypeId` 转为 PascalCase 作为 `component` 名。
...
4. **生成代码**：根据 `prototype-config.ts` 的映射，将 DemoSpec 转为 wc、react、vue 三种代码；CLI 会在写文件前按 demo 中实际使用到的组件，自动补齐 React/Vue 头部 `import`。
...
- **importPath**：根据 `prototypeId` 前缀映射到组件库（如 `shadcn` → `@prototype-libs/shadcn`）

React/Vue 示例代码的头部 import 会在 CLI 输出阶段统一补齐，默认分别指向：

- `../proto-ui/components/react`
- `../proto-ui/components/vue`
